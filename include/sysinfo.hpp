...
{
    // Constants
    static constexpr qint64           GIGABYTES_IN_BYTES{1000000000};
    static constexpr ConstexprQString CPU_NAME{"wmic cpu get name"};
    static constexpr ConstexprQString GPU_NAME{"wmic PATH Win32_videocontroller get VideoProcessor "};
    static constexpr ConstexprQString VRAM{"wmic PATH Win32_VideoController get AdapterRAM"};
    static constexpr ConstexprQString LINUX_CPU_NAME{"cat /proc/cpuinfo | grep 'model name' | uniq"};
    static constexpr ConstexprQString BIOS_KEY{R"(HKEY_LOCAL_MACHINE\HARDWARE\DESCRIPTION\System\BIOS)"};
    ...
    inline int volumesNumber()
    {
        ...
        std::for_each(std::begin(mountedVolumes), std::end(mountedVolumes),
                      [&size](const QStorageInfo &tmpVolume) {
    ...
    inline QList<QMap<QString, QString>> getDiskInformation()
    {
        ...
        for (const QStorageInfo       &storage: mountedVolumes)
        {
            if (storage.isValid() && storage.isReady())
            {
                ...
                tempMap.insert({{"Space available",
                                        QString::number(storage.bytesAvailable() / GIGABYTES_IN_BYTES) +
                                        " GB"}});
                tempMap.insert({{"Total space",
                                        QString::number(storage.bytesTotal() / GIGABYTES_IN_BYTES) +
                                        " GB"}});
